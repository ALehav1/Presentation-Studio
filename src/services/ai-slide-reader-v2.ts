 ...
 * All API calls go through your server, never exposing credentials
 */
export class AISlideReader {
  ...
  /**
   * Batch process multiple slides with rate limiting
  ...
  /**
   * Test the backend proxy connection
   * @returns true if proxy is reachable and configured
   */
  ...
  /**
   * Private method to call proxy with retry logic
   */
  ...
  /**
   * Build the appropriate prompt based on extraction needs
   */
...
VISUAL ELEMENTS: [description of charts/images if any]`;
  ...
  /**
   * Parse the API response into structured SlideContent
   */
  private parseResponse(response: Record<string, any>): SlideContent {
    ...
    // Simple parsing for MVP - enhance this based on your prompt structure
